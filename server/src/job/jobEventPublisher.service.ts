...
export class JobEventPublisher {
  ...
  static createJobEventPublisher(
    jobId: string,
    jobMetaModel: Model<JobMetaDocument>,
    jobRunModel: Model<JobRunDocument>,
  ): JobEventPublisher {
  ...
  async startJob() {
    ...
    await this.jobMetaModel
      .findByIdAndUpdate(this.jobId, {
  ...
  async skipped() {
    ...
    await this.jobRunModel
      .findByIdAndUpdate(this.currentRunId, {
        status: 'SKIPPED',
        endTime: Date.now(),
      })
      .exec();
...
export class JobEventPublisherService {
  ...
  create(jobId: string): JobEventPublisher {
    return JobEventPublisher.createJobEventPublisher(
      jobId,
      this.jobMetaModel,
      this.jobRunModel,
    );

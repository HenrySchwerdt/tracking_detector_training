...
} from './jobEventPublisher.service';
...
export abstract class JobConfiguration {
  ...
  constructor(
    private jobMetaModel: Model<JobMetaDocument>,
  ...
  async start(): Promise<void> {
    ...
    if (!jobMeta.enabled) {
      this.getLogger().log(
        "Job didn't get triggered because it was manually disabled.",
      );
      return;
    }
    ...
    try {
      ...
      this.getLogger().error(
        "Error occured at job:'" + this.job.getName() + "': ",
        e,
      );
      await publisher.failure();
    }
  ...
  onModuleInit(): void {
    ...
    const result = this.jobMetaModel
      .findOne({ jobName: this.job.getName() })
      .exec();
    result.then((value) => {
      const jobModel = value;

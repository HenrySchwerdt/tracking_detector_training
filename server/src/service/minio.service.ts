...
export class MinioService {
        ...
        this.minioClient.makeBucket(name, 'eu-central-1', (err) => {
          if (err) {
            ...
            this.minioClient.setBucketVersioning(
              name,
              versioningStateConfig,
              (error) => {
                if (error) {
                  return this.logger.error(
                    'Error updating versioning for bucket with name: ' + name,
                    err,
                  );
                }
                this.logger.log(
                  'Updated versioning for bucket with name: ' + name,
                );
              },
            );
          }
  ...
  async getModelFilesByFolderAndName(
    modelFolder: string,
    fileName: string,
  ...
  async getAllModelsAvailableToDownload(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      const data = [];
      const stream = this.minioClient.listObjects(MODEL_BUCKET, '', true);
      stream
